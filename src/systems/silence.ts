...
// A few onclick sites in this file used to chain two calls in one string
// (`closeModal(); log('...')`, `closeModal(); silLearnNumbers()`) — the
// dispatcher's data-action only carries one action name, so these composites
// stand in for those pairs (same fix as shipwalk.ts's toggleModAndInspect).
export function silCloseLog(text: string) { closeModal(); log(text); }
export function silCloseLearnNumbers() { closeModal(); silLearnNumbers(); }
export function silCloseLearnReturned() { closeModal(); silLearnReturned(); }
  ...
  if (S.loc === "verge" && c.stage === 2 && c.silenced.includes("verge")) {
    modal(`<h2>🕯 Verge Station — dark side of the band</h2>
      ...
      <div class="choices">
        ...
        <button ${actionAttr("silCloseLog", "You stay in the ring dock and keep your boots off the deck plating. Some doors want opening; this one only wants company. You can come back.")}>Not yet. Undock and breathe.</button>
      </div>`);
    return true;
  }
...
export function silAnswer() {
  ...
  replaceModal(`<h2>◆ THE ANSWER</h2>
    ...
    <div class="choices"><button class="primary" ${actionAttr("silCloseLog", "◇ The worlds are awake. The band is full of voices again — and one more voice than there used to be. It likes you.")}>Keep flying</button></div>`);
...
export function silStill() {
  ...
  replaceModal(`<h2>◆ THE STILLING</h2>
    ...
    <div class="choices"><button class="primary" ${actionAttr("silCloseLog", "◇ The band is ordinary again. Sometimes, docked and half asleep, you reach over and turn the radio on. Just to check.")}>Keep flying</button></div>`);
...
export function silSell() {
  ...
  replaceModal(`<h2>◆ THE TRANSACTION</h2>
    ...
    <div class="choices"><button class="primary" ${actionAttr("silCloseLog", "◇ 3,000 credits. The worlds woke on their own. The Union has the recording, and the Union has never once left a lever unpulled.")}>Spend it well</button></div>`);
...
export function evNumbersStation() {
  if (!S.flags.sk_numbers) {
    modal(`<h2>📻 Numbers</h2>
      ...
      <div class="choices"><button class="primary" ${actionAttr("silCloseLearnNumbers")}>Log the bearing</button></div>`);
  } else {
...
export function evReturnedShip() {
  ...
  modal(`<h2>🛸 The Returned</h2>
    ...
    <div class="choices">
      ...
      <button ${actionAttr("silCloseLog", "You give the Prodigal Anne a wide, superstitious berth. Some doors hold themselves open for a reason.")}>Burn away. Not today.</button>
    </div>`);
}
export function silBoardReturned() {
  ...
  replaceModal(`<h2>🛸 Aboard the Prodigal Anne</h2>
    ...
    <div class="choices"><button class="primary" ${actionAttr("silCloseLearnReturned")}>Take the fix. Leave the Anne.</button></div>`);
}
export function silScanReturned() {
  ...
  replaceModal(`<h2>🛸 The Standoff Scan</h2>
    ...
    <div class="choices"><button class="primary" ${actionAttr("silCloseLearnReturned")}>Log the bearing (−2 fuel)</button></div>`);
}
