...
let camYaw = 0;
let camPitch = 0;
// Dev-only chase-distance multiplier (window.__camZoom in dev builds) so mesh
// and animation work can be inspected close up without touching gameplay.
let camZoom = 1;
export function setCamZoom(z: number) { camZoom = Math.max(.25, Math.min(1.5, z)); }
if (import.meta.env.DEV) (window as any).__camZoom = setCamZoom;
...
export function mount(container: HTMLElement | null, s: WalkScene, actions: {move:(x:number,y:number)=>void;aim:(x:number,y:number)=>void;fire:()=>void}) {
  ...
  } catch (err) { console.error("walk3d mount failed, using 2D fallback:", err); teardown(); if(fallbackCanvas) fallbackCanvas.style.display="block"; }
}
