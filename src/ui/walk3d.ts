...
export function render(v:{pos:{x:number;y:number};facing:string;moving:boolean;phase:number;nearDoor:WalkDoor|null;nearActor:WalkActor|null;time:number;aim:{x:number;y:number};rolling:boolean;rollCooldown:number;projectiles:Array<{x:number;y:number}>;dummy:{x:number;y:number;hp:number;hit:number}|null;highlightKey:string|null;enemies?:Array<{x:number;y:number;hp:number;maxhp:number;hit:number;kind:string}>;foeShots?:Array<{x:number;y:number}>;playerHit?:number}){
  ...
  if(v.enemies)for(const e of v.enemies){if(e.hp<=0)continue;const frac=Math.max(0,e.hp/e.maxhp);const col=e.hit>0?"#ffffff":frac<.34?"#ff6a4d":"#c23b5a";const dm=box(.5,.9,.5,mat(col,e.hit>0?1.4:.4));dm.position.set(wx(e.x),.45,wz(e.y));actionFx.add(dm);const el=sprite(`DRONE ${Math.max(0,e.hp)}/${e.maxhp}`,"#ff9b9b",.4);el.position.set(wx(e.x),1.4,wz(e.y));actionFx.add(el);}
